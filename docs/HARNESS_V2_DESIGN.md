...
**비목표 (Phase 1~3 기준 범위 제외 — 이후 Phase에서 일부 해소됨)**
- 컴팩션 사다리 개편 (기존 80% auto-compaction 유지)
- 병렬 그래프 노드 실행 (graph discipline은 위상순 순차 실행만)
- 새 외부 크레이트 추가 (SPEC 원칙: 기존 스택 + std)
- ~~TUI 시각 개편~~ → Phase 8(§16)에서 working 패널·라이브 배선 수리로 부분 해소. 단 대규모 시각 재설계는 여전히 비목표
...
8. 기존 147+ 테스트 전부 통과 유지.
   - 현행(2026-08-27, v1.0.1 기준): src+tests 합산 `#[test]` 234개. 앞으로 Phase마다 이 수가 늘어나는 것이 정상이며, 줄면 회귀 의심.
