...
> 상태: M1~M6 완료 + 잔여 갭 보강 + **코드 품질 엔진 구축 완료** (2026-08-29, docs/agent-upgrade/07_QUALITY.md — 언어 프로파일·S3/S5 게이트·리뷰 위원회 5그룹·루브릭·설계 노트·스킬 3종, 레드팀 실증 7/설계 추적 3).
...
## 마일스톤 5 — 모델 보정·2차원 하네스·출력 견고성 (완료 ✅ 2026-08-29)

| 태스크 | 내용 | 검증(증거) | 상태 |
|---|---|---|---|
| T-M5-1 | 보정 스위트 — 9 프로브(3군: 지시 준수·구조화 출력·미니 코딩), 순수 함수 검증기(코드펜스 제거·JSON 추출 공정 채점) | 단위 테스트 4종 (프로브 구성·양/부정 케이스·펜스·정책) | ✅ |
| T-M5-2 | `rafikx calibrate` CLI + 능력 프로필 저장(model_profiles.json) | 실측: MiniMax-M3 능력 0.44 산출·저장 | ✅ |
| T-M5-3 | 2차원 하네스 — 능력 점수로 검증 정책 상향(<0.5 Strict, <0.8 Auto; 하향 없음), finish_verification 연결 | 단위 테스트 verify_upgrade + runner.rs 연결 | ✅ |
| T-M5-4 | G13 — lessons 구조화 출력 파식 실패 시 오류 첨부 1회 재요청 | lessons.rs reflect_and_save | ✅ |
| T-M5-5 | G16 — diff 크기 제한(max_diff_lines 초과 시 Rework·분해 강제) | 단위 테스트 2종 + 실측 | ✅ |

## 마일스톤 6 — 관측성·재개 완성 (완료 ✅ 2026-08-29)

| 태스크 | 내용 | 검증(증거) | 상태 |
|---|---|---|---|
| T-M6-1 | 체크포인트 재검증 — 재개 직전 마지막 Done 태스크 검증 재실행, 불일치 시 ESCALATED·중단 | 실측: 완료 태스크 결과물 훼손 → "체크포인트 불일치" 감지·중단 | ✅ |
| T-M6-2 | LEDGER 검증 이벤트 기록(orchestrator) + `rafikx plan-report` — 검증 횟수·완료 수·최고 통과 테스트·에스컬레이션 집계 | 실측: 검증 1회·완료 5개·에스컬레이션 1건 리포트 | ✅ |
| T-M6-3 | 워크 PROGRESS.md 자동 갱신 — 태스크 표 + 전체 기준 진행률 | 실측: 4/5 진행률 생성 | ✅ |
...
## 잔여 갭 보강 (M6 후속, 완료 ✅ 2026-08-29)

| 갭 | 내용 | 검증(증거) | 상태 |
|---|---|---|---|
| G15 롤백 UX | `rafikx plan-rollback <plan> <task>` — 체크포인트 커밋을 git revert 로 되돌리고(이력 보존) 태스크를 PENDING 재실행 대기로. revert 충돌 시 `--abort` 로 트리 보존 후 오류. 워크 메타는 태스크 커밋에서 제외(메타·산출물 혼합 충돌 방지, checkpoint pathspec exclude). rollback 전 소스 미커밋 변경이 있으면 안전 중단, 메타만 더러우면 정리 커밋 후 진행 | 실측: T-2 롤백 → line2 제거·PENDING, 재실행 → 복구, revert 커밋 이력 확인 | ✅ |
| G21 교훈 검증화 | `lesson_grounded` — 교훈 문장이 관측된 오류(detail)와 실제 토큰을 공유(ASCII≥4자·한글≥2자)하지 않으면 일반론·추측으로 간주해 저장 스킵 + 로그 | 단위 테스트 2종 (앵커 공유 ✓ / 무관 일반론 ✗) | ✅ |
| G20 요약 검증 구분 | 압축 요약 프롬프트 — 명령 실행으로 확인된 사실은 '✓' 접두, 모델 주장만 있는 내용은 ✓ 없이 나열해 검증 여부가 드러나게 | chat.rs compact 프롬프트 | ✅ |

전체 회귀: `cargo test` **379 passed / 0 failed**.