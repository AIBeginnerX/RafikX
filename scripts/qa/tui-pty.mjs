...
const settledReadyOutput = start.raw();
await start.page.waitForTimeout(500);
assert.equal(start.raw(), settledReadyOutput, "settled Ready emitted a TUI redraw");
...
await failed.waitFor("OpenAI server error (HTTP 500)", 8000);
await failed.waitFor("Failed", 8000);
const failedText = await failed.screenText();
assert.match(failedText.split("\n")[0], /…\/한국어-workspace-/);
assert.match(failedText, /OpenAI server error \(HTTP 500\)/);
